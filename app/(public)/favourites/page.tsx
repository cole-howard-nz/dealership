...
export default function FavouritesPage() {
  ...
  const saved = allVehicles.filter((v) => favourites.includes(v.id));

  if (loading) {
    return (
      <div className="container-wide px-4 py-20 text-center text-ink-muted">
        Loading…
      </div>
    );
  }
  ...
  if (saved.length === 0) {
    return (
      <div className="container-wide px-4 py-20 text-center">
        ...
        <Link href="/inventory" className="inline-block mt-6 bg-accent text-white rounded-md px-6 py-3 font-semibold">
          Browse Inventory
        </Link>
      </div>
    );
  ...
  return (
    <div className="container-wide px-4 py-8">
      <div className="flex items-center justify-between mb-6">
        ...
        <button onClick={clearFavourites} className="text-sm font-semibold text-ink-muted hover:text-error">
          Clear shortlist
        </button>
      </div>
