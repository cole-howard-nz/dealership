...
export const metadata: Metadata = {
  title: "Trade-In Request — Northbridge Motors Staff Portal",
};
...
export default async function TradeInRequestDetailPage({ params }: PageProps) {
  const { id } = await params;
  const session = await requirePermission("tradein.view");

  const permissions = session.user.role.permissions;
  const canUpdate = hasPermission(permissions, "tradein.update");
        ...
        <div className="flex items-start gap-3 flex-wrap">
          <div>
            <h1 className="font-heading text-2xl font-bold" style={{ color: "#13151A" }}>
              TI-{shortId} · {request.vehicleYear} {request.vehicleMake} {request.vehicleModel}
            </h1>
            <div className="flex items-center gap-3 mt-1 flex-wrap">
              ...
              <span className="text-sm" style={{ color: "#9CA3AF" }}>
                Received {formatDistanceToNow(request.createdAt, { addSuffix: true })} · {request.name}
              </span>
            </div>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-5">
          <Section title="Contact Details">
            ...
            <Field label="Email">
              <a href={`mailto:${request.email}`} className="hover:underline" style={{ color: "#E15A2C" }}>
                {request.email}
              </a>
            </Field>
            <Field label="Phone">
              <a href={`tel:${request.phone}`} className="hover:underline" style={{ color: "#E15A2C" }}>
                {request.phone}
              </a>
            </Field>
            <Field label="Preferred contact" value={request.preferredContact} />
          </Section>

          <Section title="Vehicle">
            <Field label="Make / Model" value={`${request.vehicleMake} ${request.vehicleModel}`} />
            <Field label="Year" value={String(request.vehicleYear)} />
            {request.plateNumber && <Field label="Plate" value={request.plateNumber} />}
            <Field label="Odometer" value={`${request.odometerKm.toLocaleString()} km`} />
            <Field label="Condition" value={request.condition} />
            ...
            <Field
              ...
              value={request.outstandingFinance ? "Yes" : "No"}
            />
          </Section>

          {request.vehicleDescription && (
            <Section title="Description">
              <p className="text-sm whitespace-pre-wrap leading-relaxed" style={{ color: "#5B5F6B" }}>
                {request.vehicleDescription}
              </p>
            </Section>
          )}
          ...
          <Section title="Timeline">
            <Field label="Received" value={format(request.createdAt, "d MMM yyyy, h:mm a")} />
            <Field label="Last updated" value={format(request.updatedAt, "d MMM yyyy, h:mm a")} />
          </Section>
        </div>
        ...
        <div className="space-y-4">
          ...
          <Panel title="Estimated Value">
            <EstimatedValueForm
              requestId={request.id}
              currentValue={request.estimatedValue}
              updateAction={setEstimatedValue}
              canUpdate={canUpdate}
            />
          </Panel>
